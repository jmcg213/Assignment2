...
MirrorMode::MirrorMode(int height, int width, double den)
{
  col = height;
  rows = width;
  density = den;
...
void MirrorMode::GenerateGrid(string input)   //generates a grid if user does not implement one in their file
{
  if(oput == "console")
    cin.ignore();
  string temp;
...
void MirrorMode::CountNeighbors()   //counts neighbors and then fills the future array with what is meant to happen
{
  ...
  for(int i = 0; i < col; ++i)
  {
    for(int j = 0; j < rows; ++j)
    {
      ...
      //Checking for mirror mode specifically
      if(i == 0) //checks left row reflection
      {
        if(j!=0 && j!= rows-1 && present[i][j] == 'X')
          mates++;
        if(j != 0 && present[i][j-1] == 'X')
          mates++;
        if(j != rows-1 && present[i][j+1] == 'X')
          mates++;
      }

      if(j == 0)  //checks top row reflection
      {
        if(i != 0 && i!= col-1 && present[i][j] == 'X')
          mates++;
        if(i != 0 && present[i-1][j] == 'X')
          mates++;
        if(i != col-1 && present[i+1][j] == 'X')
          mates++;
      }

      if(i == col-1)  //checks right row reflection
      {
        if(j!=0 && j!= rows-1 && present[i][j] == 'X')
          mates++;
        if(j != 0 && present[i][j-1])
          mates++;
        if(j != rows-1 && present[i][j+1])
          mates++;
      }

      if(j == rows-1)  //checks bottom row reflection
      {
        if(i != 0 && i!= col-1 && present[i][j] == 'X')
          mates++;
        if(i != 0 && present[i-1][j])
          mates++;
        if(i != col-1 && present[i+1][j])
          mates++;
      }

      if(i == 0 && j == 0 && present[i][j] == 'X')
      {
        mates += 3;
      }
      else if(i == 0 && j == rows-1 && present[i][j] == 'X')
      {
        mates += 3;
      }
      else if(i == col-1 && j == 0 && present[i][j] == 'X')
      {
        mates += 3;
      }
      else if(i == col-1 && j == rows-1 && present[i][j] == 'X')
      {
        mates += 3;
      }
...
void MirrorMode::UpdateGrid()   //will iterate through a generation and display the new generation
{
  bool loopp = true;
  bool loopf = true;
  bool dashAll = true;
...
string MirrorMode::SetOutputType()
{
  string fileName;
  string output;
  while(true)
